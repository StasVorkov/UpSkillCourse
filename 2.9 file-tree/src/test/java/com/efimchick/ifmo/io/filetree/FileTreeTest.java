...
import com.efimchick.ifmo.io.filetree.tree.FileTreeDataHolder;
...
public class FileTreeTest {
    ...
    public void testEmptyCases() {
        assertTrue("null case failure", new FileTreeDataHolder().printFileTree(null).isEmpty());
        assertTrue("nonexistent path case failure", new FileTreeDataHolder().printFileTree(Paths.get("some", " non", "existent", "path")).isEmpty());
    }
    ...
    private String tree(final String... subPath) {
        return new FileTreeDataHolder().printFileTree(Paths.get("src/test/resources", subPath)).orElse(null);
    }
