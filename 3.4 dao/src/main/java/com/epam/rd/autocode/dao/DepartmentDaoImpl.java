...
public class DepartmentDaoImpl implements DepartmentDao {
	...
	public List<Department> getAll() {
		...
		try (Connection connection = connectionSource.createConnection();
			...
			while (resultSet.next()) {
				department = new Department(new BigInteger(resultSet.getString(ID)),
						resultSet.getString(NAME),
						resultSet.getString(LOCATION));
				list.add(department);
			}
	...
	public Department save(Department department) {
		if (isDepartmentAlreadyExist(department)) {
			update(department);
		} else {
			insert(department);
		}
	...
	private boolean isDepartmentAlreadyExist(Department department) {
		return getById(department.getId()).isPresent();
	}
